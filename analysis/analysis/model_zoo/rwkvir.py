...
from basicsr.module.base.vrwkv4 import Block as RWKV
...
class RWKVIRv4(nn.Module):
    def __init__(self, img_size=64, patch_size=1, in_chans=3,
                 embed_dim=96, depths=[6, 6, 6, 6], mlp_ratio=4.,
                 shift_mode='q_shift', init_mode='local',
                 channel_gamma=1/4, shift_pixel=1, hidden_rate=4,
                 init_values=None, post_norm=False, key_norm=False, with_cp=False,
                 drop_rate=0., drop_path_rate=0., act_layer=nn.GELU,
                 norm_layer=nn.LayerNorm, ape=False, patch_norm=True,
                 use_checkpoint=False, upscale=2, img_range=1., upsampler='', resi_connection='1conv',
                 **kwargs):
        super(RWKVIRv4, self).__init__()
        num_in_ch = in_chans
        num_out_ch = in_chans
        num_feat = 64
...
def buildRWKVIR(upscale=2):
    return RWKVIRv4(img_size=(64, 64), depths=[6, 6, 6, 6, 6, 6], 
                   mlp_ratio=2., patch_size=8,
                   img_range=1, embed_dim=192, upscale=upscale,
...
#                    upsampler='pixelshuffledirect', resi_connection='1conv',).to('cuda')
#     print(model)
#     print(height, width)
